...
num_series = 2 if input(
    'Enter "y" if two data series are being used: ') == 'y' else 1
...
for series_index in range(num_series):
    ...
    series_name = input(
        'Enter the name of data series {}: '.format(series_index))
...
for subplot, series_name, color in zip(subplots, values_by_trial.keys(), COLORS):
    ...
    subplot.set_ylabel(series_name, fontsize=AXIS_LABEL_SIZE,
                       labelpad=AXIS_LABEL_PADDING)
    # Set the X axis label with the specified X axis label
    subplot.set_xlabel(x_label, fontsize=AXIS_LABEL_SIZE,
                       labelpad=AXIS_LABEL_PADDING)
